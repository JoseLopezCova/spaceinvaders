...
public class Main extends ApplicationAdapter {
    ...
    public void create() {
        ...
        player = new NaveAmiga(500, 200, 6, 3, true);
    ...
    public void render() {
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (iPosYClicked > player.getY()) {
                DisparoAliado disparoAliado = new DisparoAliado(player.getX(), player.getY() + 20, 10, 3); // vamos para arriba
        ...
        if (!bGanamos && Math.random() > 0.99) {
            ...
            enemigo.setX(dado.nextInt(Gdx.graphics.getWidth()));
            enemigo.setY(dado.nextInt(Gdx.graphics.getHeight()));
        }

        // Generar disparos enemigos con cierta probabilidad
        if (!bGanamos && Math.random() > 0.99) {
            DisparoEnemigo disparoEnemigo = new DisparoEnemigo(enemigo.getX(), enemigo.getY() -20, 10, 2);
